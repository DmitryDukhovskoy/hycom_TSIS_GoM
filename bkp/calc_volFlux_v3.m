% HYCOM-TSIS
% hindcasts: data assimilative runs
%            with PIES - deep obs of T/S fields under LC
%            noPIES
%
% forecasts: 3 month free runs started every month
...
HR=0;
%esim='GLfreerun';  % free run with GLROYS OB
esim='PIES'; % hindcast data assimilative runs with deep PIES obs
%esim='noPIES';
%esim='fcst';
%esim='freerun';
%esim='adj';
...
imo = 6; % only for forecasts - start month of fcst, 
         % otherwise ignored
...
switch(esim)
 ...
 case('fcst');
  fmat = sprintf('%s%s_VolTrt_%s_%4.4i%2.2i.mat',...
   pthmat,esim,xsct_name,ys,imo);
 case('freerun')
  fmat = sprintf('%s%s_VolTrt_%s_%4.4i-%4.4i.mat',...
   pthmat,esim,xsct_name,ys,ye);
 case('adj');
...
%keyboard
  ...
  % Save mat file
    ...
    switch(esim)
     case('PIES'),
      pthi=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/%4.4i_PIES/',iyr);
     case('noPIES');
      pthi=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/%4.4i_noPIES/',iyr);
     case('fcst');
      pthi=sprintf('/Net/gleam/abozec/HYCOM/TSIS/IASx0.03/forecast/%4.4i%2.2i/',...
     ys,imo);
     case('freerun');
      pthi=sprintf('/Net/gleam/abozec/HYCOM/TSIS/IASx0.03/freerun/');
     case('adj');
      pthi=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/%4.4i_adj/',iyr);
     case('GLfreerun');
      pthi=sprintf('/Net/gleam/dmitry/hycom/TSIS/IASx0.03/output/2011_GLfreerun/');
    end
  ...
  % Layer thickness is intetrpolated into V point
  % from j and j-1 grid cells
      ...
      if strcmp(OB,'S') | strcmp(OB,'N')
        ...
        for ip=1:length(II)
          i0=II(ip);
          h3(:,ip)=0.5*(THK(:,j0,i0)+THK(:,j0-1,i0));
        end;
      end
  % For U normal to the bndry:
  % Layer thickness is intetrpolated into U point
  % from i and i-1 grid cells
      if strcmp(OB,'E') | strcmp(OB,'W');
        ...
        for ip=1:length(II)
          j0=II(ip);
          h3(:,ip)=0.5*(THK(:,j0,i0)+THK(:,j0,i0-1));
        end
      end   % if strcmp
